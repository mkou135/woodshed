...
const LEGEND: LegendEntry[] = [
  {
    ...
    swatch: '#c4641d',
    meaning: 'Where the engine hears a phrase begin — the breath-to-breath unit. A half-opacity tick is a structural boundary (a chorus start) rather than one the music itself forced.',
    parameters: 'Boundary strength per gap = min(1, 0.6·rest + 0.45·length + 0.25·leap); a phrase ends when the total reaches 0.45 with a real rest present (rests under a 16th are articulation; a quarter or more counts in full). Scored 82.5 F1 against the 456-solo Weimar Jazz Database.',
  },
  {
    vector: 'idea',
...
export function annotationExportHtml(title: string, svgMarkup: string, items: OverlayItem[]): string {
  const sections = VECTOR_ORDER.map((vector) => {
...
<table>${rows.map((i) => `<tr><td class="id">${esc(i.id)}</td><td class="what">${esc(i.label)}</td><td class="where">${esc(i.where)}</td><td class="detail">${esc(i.detail)}</td></tr>`).join('\n')}</table>`
  ...
  const legend = LEGEND.map((l) => `<div class="entry">
...
<title>${esc(title)} — engine annotations</title>
<style>
  ...
  ${SVG_CSS}
  @media print { .score { border: none; overflow: visible } }
</style></head><body>
